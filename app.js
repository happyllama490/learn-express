const express = require('express'); // Expreuploadss 모듈을
...
// express.static -> 장작 파일들을 제공하는 라우터 역할
// public 디렉토리 안에 있는 파일들을 제공 -> 직접 읽어서 전송하지 않아도 됨.
app.use('/', express.static(path.join(__dirname, 'public')));

// 요청 본문에 있는 데이터 해석 -> req.body 객체로 만들어주는 미들웨어
app.use(express.json());
    // JSON 데이터를 JabaScript 객체로 변환 -> req.body 객체에 저장
...
app.use(session({
    ...
    // cookie -> 세션 쿠키에 대한 설정
        // httpOnly: true -> 클라이언트에서 쿠키를 학인할 수 없음.
